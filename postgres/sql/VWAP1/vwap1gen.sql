...
begin
    ...
    SELECT r.price AS ineqkey1
    FROM aggbids r
    UNION
    SELECT s.price AS ineqkey1
    FROM aggbids s
    ...
    SELECT x.ineqkey1, SUM(s.volume) AS agg
    FROM cross_b2 x
             LEFT JOIN aggbids s ON x.ineqkey1 = s.price
    GROUP BY x.ineqkey1
...
create procedure construct_rt_b2(height_d1 integer, bf_d1 integer)
    language plpgsql as
$$
begin
    delete from rt_b2;
    insert into rt_b2
    SELECT 0, dense_rank() over ( ORDER BY price ASC ) - 1, price, price, SUM(volume)
    FROM aggbids
    GROUP BY price;
...
begin
    ...
    for v1 in reverse height_d1..0
        loop
            ...
            into row1
            from rt_b2
            where (lvl1 = v1 AND (upper1 < _outer.price AND upper1 < lower1_min))
            ...
            into row1
            from rt_b2
            where (lvl1 = v1 AND (upper1 < _outer.price AND upper1 > upper1_max))
            ORDER BY upper1 ASC
            limit 1;
