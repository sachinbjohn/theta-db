...
FROM aggbidsR r, groups_bS UNION SELECT DISTINCT s.price AS ineqkey1, s.time AS gbykey1
FROM aggbidsS s
...
create temp table cube_bS on commit drop as 
 SELECT ineqkey1, gbykey1, SUM(agg) OVER (partition by gbykey1 ORDER BY ineqkey1 ASC rows between unbounded preceding and 1 preceding) AS agg
FROM cube_bS_delta1
ORDER BY ineqkey1 ASC, gbykey1 ASC;
...
create type cube_bS_aggtype as (gbykey1 double precision,aggbS double precision);
...
create type rt_bS_aggtype as (gbykey1 double precision,aggbS double precision);
...
insert into rt_bS
SELECT time, ceil(log(2, SUM(1) OVER (partition by time  ))), dense_rank() over(partition by time ORDER BY price ASC ) - 1, price, price, SUM(agg)
FROM aggbidsS
GROUP BY time, price;
...
create temp table rt_bS_dst on commit drop as 
 SELECT gbykey1, (lvl1 - 1) AS lvl1, (rnk1 / bf_d1) AS rnk1, MIN(MIN(lower1)) OVER (partition by gbykey1,(rnk1 / bf_d1)  ) AS lower1, MAX(MAX(upper1)) OVER (partition by gbykey1,(rnk1 / bf_d1)  ) AS upper1, SUM(agg) AS agg
FROM rt_bS_src
WHERE lvl1 > 0
...
FROM aggbidsS r UNION SELECT DISTINCT s.price AS ineqkey1
FROM aggbidsR s
...
create temp table rt_bR_dst on commit drop as 
 SELECT (lvl1 - 1) AS lvl1, (rnk1 / bf_d1) AS rnk1, MIN(MIN(lower1)) OVER (partition by (rnk1 / bf_d1)  ) AS lower1, MAX(MAX(upper1)) OVER (partition by (rnk1 / bf_d1)  ) AS upper1, SUM(agg) AS agg
FROM rt_bR_src
WHERE lvl1 > 0
