...
FROM aggbids r UNION SELECT DISTINCT s.price AS ineqkey2
FROM aggbids s
...
FROM cross_b2 x
  LEFT JOIN aggbids s ON (x.ineqkey1 = s.time AND x.ineqkey2 = s.price)
...
insert into rt_b2
SELECT 0, dense_rank() over( ORDER BY time ASC ) - 1, time, time, 0, dense_rank() over(partition by time ORDER BY price ASC ) - 1, price, price, SUM(agg)
FROM aggbids
GROUP BY time, price;
...
SELECT v1, (rnk1 / bf_d1), MIN(MIN(lower1)) OVER (partition by (rnk1 / bf_d1)  ), MAX(MAX(upper1)) OVER (partition by (rnk1 / bf_d1)  ), 0, dense_rank() over(partition by (rnk1 / bf_d1) ORDER BY lower2 ASC ) - 1, lower2, lower2, SUM(agg)
FROM rt_b2
WHERE lvl1 = (v1 - 1)
...
truncate rt_b2_new;
insert into rt_b2_new
SELECT lvl1, rnk1, lower1, upper1, v2, (rnk2 / bf_d2), MIN(MIN(lower2)) OVER (partition by lvl1,rnk1,(rnk2 / bf_d2)  ), MAX(MAX(upper2)) OVER (partition by lvl1,rnk1,(rnk2 / bf_d2)  ), SUM(agg)
FROM rt_b2
WHERE lvl2 = (v2 - 1)
