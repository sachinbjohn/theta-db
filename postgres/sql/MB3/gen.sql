...
begin
    ...
    insert into rt_b2
    SELECT eqkey1, ceil(log(2, (MAX(rnk1) OVER (partition by eqkey1 ) + 1))), rnk1, lower1, upper1, agg
    FROM (
             SELECT time                                                          AS eqkey1,
    ...
    loop
        ...
        SELECT eqkey1,
               (lvl1 - 1)                                                  AS lvl1,
               (rnk1 / bf_d1)                                              AS rnk1,
               MIN(MIN(lower1)) OVER (partition by eqkey1,(rnk1 / bf_d1) ) AS lower1,
               MAX(MAX(upper1)) OVER (partition by eqkey1,(rnk1 / bf_d1) ) AS upper1,
               SUM(agg)                                                    AS agg
        FROM rt_b2_src
        WHERE lvl1 > 0
