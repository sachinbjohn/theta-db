...
begin
    ...
    SELECT DISTINCT r.time AS ineqkey1, gbykey1
    FROM aggbids r,
         groups_b2
    UNION
    SELECT DISTINCT s.time AS ineqkey1, s.time AS gbykey1
    FROM aggbids s
    ...
    FROM cross_b2 x
             LEFT JOIN aggbids s ON (x.ineqkey1 = s.time AND x.gbykey1 = s.time)
...
begin
    ...
    FROM (
             SELECT time                                                         AS gbykey1,
                    dense_rank() over (partition by time ORDER BY time ASC ) - 1 AS rnk1,
                    time                                                         AS lower1,
                    time                                                         AS upper1,
                    SUM(agg)                                                     AS agg
             FROM aggbids
    ...
    for row0 in
        ...
        loop
            ...
            loop
                ...
                into row1
                from rt_b2
                where ((gbykey1 = row0.gbykey1 AND lvl1 = v1) AND (upper1 < _outer.time AND upper1 < lower1_min))
                ...
                into row1
                from rt_b2
                where ((gbykey1 = row0.gbykey1 AND lvl1 = v1) AND (upper1 < _outer.time AND upper1 > upper1_max))
                ORDER BY upper1 ASC
                limit 1;
