...
begin
    ...
    select b1.time, sum(b2.agg) as agg from aggbids b1 left join aggbids b2
    on b2.time < b1.time
    group by b1.time;

    insert into result
    select b1.price, b1.time, b1.volume, agg
    from bids b1 join cumaggbids b2 on b2.time = b1.time;
end;
$$;
...
begin
    ...
    call construct_rt_b2(lt);
    insert into result
    select b1.*, (f).*
    from bids b1,
         lateral (select lookup_rt_b2(b1.*, lt) as f offset 0) func;
