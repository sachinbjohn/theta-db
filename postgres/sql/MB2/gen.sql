...
begin
    ...
    insert into rt_b2
    SELECT ceil(log(2, (MAX(rnk1) OVER ( ) + 1))), rnk1, lower1, upper1, agg
    FROM (
             SELECT dense_rank() over ( ORDER BY time ASC ) - 1 AS rnk1, time AS lower1, time AS upper1, SUM(agg) AS agg
             FROM aggbids
             GROUP BY time
         ) sub;
