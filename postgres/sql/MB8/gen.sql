...
FROM aggbidsR r UNION SELECT DISTINCT s.time AS ineqkey1
FROM aggbidsS s
ORDER BY ineqkey1 ASC;

create temp table domain_bS_dim2 on commit drop as 
 SELECT DISTINCT r.price AS ineqkey2
FROM aggbidsR r UNION SELECT DISTINCT s.price AS ineqkey2
FROM aggbidsS s
...
create temp table cube_bS_delta1 on commit drop as 
 SELECT ineqkey1, ineqkey2, SUM(agg) OVER (partition by ineqkey1 ORDER BY ineqkey2 ASC rows between unbounded preceding and 1 preceding) AS agg
FROM cube_bS_delta2
...
create type cube_bS_aggtype as (aggbS double precision);
...
create type rt_bS_aggtype as (aggbS double precision);
...
insert into rt_bS
SELECT ceil(log(2, SUM(1) OVER (  ))), dense_rank() over( ORDER BY time ASC ) - 1, time, time, ceil(log(2, SUM(1) OVER (partition by time  ))), dense_rank() over(partition by time ORDER BY price ASC ) - 1, price, price, SUM(agg)
FROM aggbidsS
GROUP BY time, price;
...
alter table rt_bS_dst rename to rt_bS_src;
end loop;
...
into row2
from rt_bS
where (((lvl1 = v1 AND lvl2 = v2) AND (lower1 = row1.lower1 AND upper1 = row1.upper1)) AND (upper2 < _outer.price AND upper2 < lower2_min))
...
into row2
from rt_bS
where (((lvl1 = v1 AND lvl2 = v2) AND (lower1 = row1.lower1 AND upper1 = row1.upper1)) AND (upper2 < _outer.price AND upper2 > upper2_max))
ORDER BY upper2 ASC
limit 1;
...
create temp table domain_bR_dim1 on commit drop as 
 SELECT DISTINCT r.time AS ineqkey1
FROM aggbidsS r UNION SELECT DISTINCT s.time AS ineqkey1
FROM aggbidsR s
ORDER BY ineqkey1 DESC;

create temp table domain_bR_dim2 on commit drop as 
 SELECT DISTINCT r.price AS ineqkey2
FROM aggbidsS r UNION SELECT DISTINCT s.price AS ineqkey2
FROM aggbidsR s
...
create temp table cube_bR on commit drop as 
 SELECT ineqkey1, ineqkey2, SUM(agg) OVER (partition by ineqkey2 ORDER BY ineqkey1 DESC rows between unbounded preceding and 1 preceding) AS agg
FROM cube_bR_delta1
ORDER BY ineqkey1 DESC, ineqkey2 DESC;
...
create type cube_bR_aggtype as (aggbR double precision);
...
create type rt_bR_aggtype as (aggbR double precision);
...
alter table rt_bR_dst rename to rt_bR_src;
end loop;
...
into row2
from rt_bR
where (((lvl1 = v1 AND lvl2 = v2) AND (lower1 = row1.lower1 AND upper1 = row1.upper1)) AND (lower2 > _outer.price AND lower2 < lower2_min))
...
into row2
from rt_bR
where (((lvl1 = v1 AND lvl2 = v2) AND (lower1 = row1.lower1 AND upper1 = row1.upper1)) AND (lower2 > _outer.price AND lower2 > upper2_max))
ORDER BY lower2 ASC
limit 1;
