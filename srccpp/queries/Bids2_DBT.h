...
namespace dbtoaster {
  ...
  struct data_t : tlq_t {
    ...
    void on_batch_update_BIDS(const vector<BatchMessage<BidsAdaptor::MessageType, int>::KVpair>& batch) {
      ...
      { //foreach
        COUNTBIDS1BIDS1_DELTA_entry* e3 = COUNTBIDS1BIDS1_DELTA.head;
        while (e3) {
          const DoubleType b1_t = e3->B1_T;
          const long b1_id = e3->B1_ID;
          const long b1_broker_id = e3->B1_BROKER_ID;
          const DoubleType b1_volume = e3->B1_VOLUME;
          const DoubleType b1_price = e3->B1_PRICE;
          const long v3 = e3->__av;
          COUNTBIDS1.addOrDelOnZero(se3.modify(b1_t, b1_id, b1_broker_id, b1_volume, b1_price), v3);
          e3 = e3->nxt;
        }
      }
      
      { //foreach
        COUNTBIDS1_L2_1BIDS1_DELTA_entry* e4 = COUNTBIDS1_L2_1BIDS1_DELTA.head;
        while (e4) {
          const DoubleType b2_t = e4->B2_T;
          const DoubleType b2_price = e4->B2_PRICE;
          const long v4 = e4->__av;
          COUNTBIDS1_L2_1.addOrDelOnZero(se4.modify(b2_t, b2_price), v4);
          e4 = e4->nxt;
        }
      }
      ...
      COUNT.clear();
      { //foreach
        COUNTBIDS1_entry* e5 = COUNTBIDS1.head;
        while (e5) {
          const DoubleType b1_t = e5->B1_T;
          const long b1_id = e5->B1_ID;
          const long b1_broker_id = e5->B1_BROKER_ID;
          const DoubleType b1_volume = e5->B1_VOLUME;
          const DoubleType b1_price = e5->B1_PRICE;
          const long v5 = e5->__av;
          int agg1 = 0;
          long agg2 = 0L;
          { //foreach
            COUNTBIDS1_L2_1_entry* e6 = COUNTBIDS1_L2_1.head;
            while (e6) {
              const DoubleType b2_t = e6->B2_T;
              const DoubleType b2_price = e6->B2_PRICE;
              const long v6 = e6->__av;
              if (b2_price > b1_price) {
                (/*if */(b1_t >= b2_t) ? agg2 += v6 : 0L);
              }
              e6 = e6->nxt;
            }
          }
          long l1 = agg2;
          (/*if */(l1 == 0) ? agg1 += 1 : 0);
          COUNT.addOrDelOnZero(se5.modify(b1_t, b1_id, b1_broker_id, b1_volume, b1_price), (v5 * agg1));
          e5 = e5->nxt;
        }
      }
    }
  ...
  private:
      ...
      COUNTBIDS1_entry se3;
      COUNTBIDS1_L2_1_entry se4;
      COUNT_entry se5;
