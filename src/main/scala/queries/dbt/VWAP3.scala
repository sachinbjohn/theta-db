...
object VWAP3 {
  ...
  def execute(args: Array[String], f: List[Any] => Unit) = 
    bench(args, (dataset: String, parallelMode: Int, timeout: Long, batchSize: Int) => run[VWAP3](
      Seq(
  ...
  def onBatchUpdateBIDS(DELTA_BIDS:M3Map[TDLLDD, Long]) {
    ...
    VWAPBIDS1.foreach { (k7, v7) =>
      ...
      VWAPBIDS1_L1_1.foreach { (k8, v8) =>
        val b2_t = k8._1;
        val b2_price = k8._2;
        (if (b2_t >= (-10 + b1_t) && b1_t >= b2_t && b1_price > b2_price) agg1 += v8 else ())
      }
      val l1 = agg1;
      var agg2: Double = 0.0
      VWAPBIDS1_L2_1.foreach { (b3_t, v9) =>
        (if (b1_t >= b3_t && b3_t >= (-10 + b1_t)) agg2 += v9 else ())
      }
      val l2 = (agg2 * 0.25);
...
class VWAP3 extends VWAP3Base with Actor {
