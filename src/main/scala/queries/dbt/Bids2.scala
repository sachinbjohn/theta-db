...
object Bids2 {
  ...
  def execute(args: Array[String], f: List[Any] => Unit) = 
    bench(args, (dataset: String, parallelMode: Int, timeout: Long, batchSize: Int) => run[Bids2](
      Seq(
...
class Bids2Base {
  ...
  def onBatchUpdateBIDS(DELTA_BIDS:M3Map[TDLLDD, Long]) {
    ...
    COUNTBIDS1BIDS1_DELTA.foreach { (k3, v3) =>
      val b1_t = k3._1;
      val b1_id = k3._2;
      val b1_broker_id = k3._3;
      val b1_volume = k3._4;
      val b1_price = k3._5;
      COUNTBIDS1.add(new TDLLDD(b1_t, b1_id, b1_broker_id, b1_volume, b1_price), v3)
    }
    COUNTBIDS1_L2_1BIDS1_DELTA.foreach { (k4, v4) =>
      val b2_t = k4._1;
      val b2_price = k4._2;
      COUNTBIDS1_L2_1.add(new TDD(b2_t, b2_price), v4)
    }
    COUNT.clear()
    COUNTBIDS1.foreach { (k5, v5) =>
      val b1_t = k5._1;
      val b1_id = k5._2;
      val b1_broker_id = k5._3;
      val b1_volume = k5._4;
      val b1_price = k5._5;
      var agg1: Long = 0L
      var agg2: Long = 0L
      COUNTBIDS1_L2_1.foreach { (k6, v6) =>
        val b2_t = k6._1;
        val b2_price = k6._2;
        (if (b2_price > b1_price && b1_t >= b2_t) agg2 += v6 else ())
      }
      val l1 = agg2;
      (if (l1 == 0) agg1 += 1L else ())
      COUNT.add(new TDLLDD(b1_t, b1_id, b1_broker_id, b1_volume, b1_price), (v5 * agg1))
    }
  }
...
class Bids2 extends Bids2Base with Actor {
